...
data <- read_excel("StoryContent 1.xlsx")
...
lda_model_fake <- LDA(dtm_fake, k = 4, control = list(seed = 1234))  # k = number of topics, set seed for reproducibility

# Topic Modeling for real stories
lda_model_real <- LDA(dtm_real, k = 4, control = list(seed = 1234))  # k = number of topics, set seed for reproducibility
